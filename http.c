/*
 *  http.c -- a small lightweight http server
 *
 *  (borrowed from didiwiki)
 *
  ...
  while( *data )
    {
      ...
      if( *data == '=' )
      ...
      else
        {
          ...
          http_request_add_param(req, key, 0);
        }
    }
}
...
void
http_response_set_content_type(HttpResponse *res, const char *type)
{
...
void
http_response_append_header(HttpResponse *res, const char *header)
{
...
http_response_printf(HttpResponse *res, const char *format, ...)
{
  ...
  if ((res->data_len + strlen(tmp)) < res->data_len_alloced) {
    if (res->data_len)
      memcpy(res->data + res->data_len, tmp, strlen(tmp));
    else
      memcpy(res->data, tmp, strlen(tmp));
  } else if (res->data_len == 0) {          /* no data printed yet */
    res->data = malloc(strlen(tmp));
    memcpy(res->data, tmp, strlen(tmp));
    res->data_len_alloced = strlen(tmp);
  } else {
  ...
  res->data_len += strlen(tmp);
  free(tmp);
}

void
http_response_write(HttpResponse *res, const void *data, size_t len)
{
  if ((res->data_len + len) < res->data_len_alloced) {
    memcpy(res->data + res->data_len, data, len);
  } else if (res->data_len == 0) {          /* no data printed yet */
    res->data = malloc(len);
    memcpy(res->data, data, len);
    res->data_len_alloced = len;
  } else {
    res->data = realloc(res->data, res->data_len + len );
    memcpy(res->data + res->data_len, data, len);
    res->data_len_alloced = res->data_len + len;
  }
...
#if 0
  printf("Content-Type: %s; charset=UTF-8\r\n", res->content_type == 0 ? "text/html" : res->content_type);
#else
...
*/
HttpRequest*
http_server(const char *name, int iPort)
{
  ...
  fprintf(stderr,"%s started. Please point your browser at http://localhost:%i\n", name, iPort+n);
